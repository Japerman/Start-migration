...
/**
 * @group unit
 * @coversDefaultClass Start\Api\StartApplication
 *
...
class StartApplicationTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function test__construct()
    {
        ...
        $this->assertInstanceOf("Start\Api\StartApplication", $app);
    }
    ...
    public function testUp()
    {
        ...
        $container['start.migrator'] = $this->getMigrator($adapter, $container, $this->output, 1, 0);
        
        $app = new StartApplication($container, $this->output);
    ...
    public function testDown()
    {
        ...
        $container = $this->getContainer($adapter, $migrations, $this->getTempDir());
        $container['start.migrator'] = $this->getMigrator($adapter, $container, $this->output, 0, 1);
    ...
    /**
     * @param array $version
     * @return Start\Adapter\AdapterInterface mock
     */
    protected function getAdapter(array $versions = array())
    {
        $adapter = $this->getMock('Start\Adapter\AdapterInterface');
     ...
     * @return Start\Migration\Migrator mock
     */
    protected function getMigrator($adapter, $container, $output, $times_up, $times_down)
    {
        $migrator = $this->getMock("Start\Migration\Migrator", array("up", "down"), array($adapter, $container, $output));
        if ($times_up > 0) {
            $migrator->expects($this->exactly($times_up))
                ->method("up")
                ->with($this->isInstanceOf("Start\Migration\Migration"));
        }
    ...
    protected function getContainer($adapter, array $migrations, $migrations_path)
    {
        return new \ArrayObject(array(
            ...
            'start.migrations' => $migrations,
            'start.migrations_path' => $migrations_path
        ));
    }
