...
/**
 * The start application for API processing
 *
 * Usage:
 * <code>
 * $container = include_once "/full/path/to/start.php";
 * $output = new Symfony\Component\Console\Output\OutputInterface\BufferedOutput;
 * $start = new Start\Api\StartApplication($container, $output);
 * $start->up(); // upgrade to latest version
...
class StartApplication
{
    ...
    public function __construct(\ArrayAccess $container, OutputInterface $output)
    {
        ...
        $this->output = $output;
        if (!isset($this->container['start.migrator']))
            $this->container['start.migrator'] = new Migration\Migrator($container['start.adapter'], $this->container, $this->output);
        
        $migrations = array();
        if (isset($this->container['start.migrations'])) {
            $migrations = $this->container['start.migrations'];
            foreach ($migrations as &$migration) {
                $migration = realpath($migration);
            }
            unset($migration);
        }
        if (isset($this->container['start.migrations_path'])) {
            $migrationsPath = realpath($this->container['start.migrations_path']);
    ...
    {
        $adapter = ! empty($this->container['start.adapter']) ? $this->container['start.adapter'] : null;

        if ($adapter == null) {

            throw new RuntimeException("The container must contain a start.adapter key!");
        }

        if (!$adapter->hasSchema()) {

            $this->container['start.adapter']->createSchema();
        }

        foreach ($this->getMigrations($this->getVersion(), $version) as $migration) {
            $this->container['start.migrator']->up($migration);
        }
    }
    ...
    public function down($version = 0)
    {
        ...
        foreach ($this->getMigrations($this->getVersion(), $version) as $migration) {
            $this->container['start.migrator']->down($migration);
        }
    }
     ...
     * @return array An array of Start\Migration\Migration objects to process
     */
     ...
     * @return array An array of Start\Migration\Migration objects
     */
    ...
    {
        $versions = $this->container['start.adapter']->fetchAll();
        sort($versions);
